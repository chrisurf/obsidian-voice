...
- **Skip content inside markers** — remove everything wrapped in chosen markers before speaking, markers included. Pick from common presets — English `( )`, full-width Chinese `（ ）`, `[ ]`, `【 】`, `《 》`, `{ }`, and Obsidian `%% comments %%` — or add your own literal marker pairs (e.g. `(( … ))`). Markdown delimiters work as pairs too — `** … **` skips bold, `* … *` italics, `` ` … ` `` inline code, and `[[ … ]]` wiki links. A master switch leaves notes completely untouched when off; unmatched markers are never cut, and fenced code blocks are always left as-is.
