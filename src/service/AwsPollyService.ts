 ...
 *
 * Current state: Using placeholder SSMLTagger
 * - SSMLTagger: Now only escapes XML and wraps in <speak> tags
 * - All SSML enhancement removed
 *
 * TODO: Integrate new MarkdownToSSMLProcessor pipeline
 * - Replace SSMLTagger with full pipeline processing
...
export class AwsPollyService {
   ...
   */
  async playSSMLAudio(ssml: string, speed?: number): Promise<void> {
    try {
      ...
      if (ssml.length > 2500) {
        ...
        await this.playSSMLChunks(chunks, speed);
      } else {
        ...
        if (ssml === this.synthesizeInput.Text && !this.voiceChanged) {
          ...
          await this.callPollySSML(ssml, speed);
        }
      }
   ...
   */
  private async callPollySSML(ssml: string, speed?: number): Promise<void> {
    if (this.isLoading) {
      this.cancelLoading();
    }
